...
$sql = "SELECT aid, designation, department,name,bid FROM administrator";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "aid: " . $row["aid"]."designation: " . $row["designation"]."department: " . $row["department"].  "name: " . $row["name"].  "bid: " . $row["bid"]. "<br>";
    }
} else {
    echo "0 results";
}
